...
public class GroundFactory : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        lf = gameObject.AddComponent<ListFactory>();
        groundList = lf.GenerateGroundChunkList();
    }
    #endregion
    ...
    {
        int randomPick = UnityEngine.Random.Range(0, groundList.Count - 1);
        GameObject randomChunk = groundList[randomPick];
        Transform groundChunk = randomChunk.transform;

        // Spawn the Transform at the last end of section location
        Transform lastGroundEnd_Right = SpawnGroundChunk_Right(groundChunk, groundEnd_Right);

        // Find the next end of section in the new Transform
        groundEnd_Right = lastGroundEnd_Right.Find("GroundEnd_Right").position;

    }
    public Transform SpawnGroundChunk_Right(Transform groundChunk, Vector3 nextPosition)
    {
        // Spawn the Platform Chunk
        Transform nextGroundChunk_Right = Instantiate(groundChunk, nextPosition, Quaternion.identity);
